...
function printAllSongs() {
  return console.log(...beyonceHash.hits, ...beyonceHash.movies)
}
...
function hitSongTitles() {
  return beyonceHash.hits.map(hitSong => hitSong.title)
}

// 4. Return an array of all Beyonce's fierceness ratings
function allFiercenessRatings() {
  return beyonceHash.hits.map(fiercenessRating => fiercenessRating.fierceness)
}
...
function getSong(song) {
  return beyonceHash.hits.find(hit => hit.title = song),
    beyonceHash.movies.find(movie => movie.title = song)
}
